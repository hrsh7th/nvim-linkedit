...
---@alias linkedit.kit.Vim.Keymap.Keys { keys: string, remap?: boolean }
---@alias linkedit.kit.Vim.Keymap.KeysSpecifier string|linkedit.kit.Vim.Keymap.Keys

---@param keys linkedit.kit.Vim.Keymap.KeysSpecifier
---@return linkedit.kit.Vim.Keymap.Keys
...
---Send keys.
---@param keys linkedit.kit.Vim.Keymap.KeysSpecifier|linkedit.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("linkedit.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("linkedit.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
